...
import { Search, Users, User, Mail, ChevronDown, Check, Loader2 } from "lucide-react"
    ...
    <div className={`relative ${className}`} ref={dropdownRef}>
      <div className="relative">
        ...
        <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex items-center gap-1">
          {(isLoading || isSearching) && <Loader2 className="w-4 h-4 text-[#97CBDC]/70 animate-spin" />}
          {selectedContact && !isLoading && !isSearching && (
            <button onClick={clearSelection} className="text-[#97CBDC]/70 hover:text-[#97CBDC] transition-colors">
              <Check className="w-4 h-4" />
            </button>
          )}
          <ChevronDown className={`w-4 h-4 text-[#97CBDC]/70 transition-transform ${isOpen ? "rotate-180" : ""}`} />
        </div>
      </div>
              ...
              <div className="p-4 text-center text-[#97CBDC]/70">
                {contacts.length === 0 ? (
                  <div>
                    ...
                    <p className="text-xs mt-1">Add contacts to use this feature</p>
                  </div>
                ) : (
                  <div>
              ...
              <div className="py-2">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <User className="w-4 h-4 text-[#97CBDC]/70 flex-shrink-0" />
                          <span className="font-medium text-[#97CBDC] truncate">{contact.fullName}</span>
                        ...
                        <div className="text-xs text-[#97CBDC]/70 font-mono truncate">{contact.walletAddress}</div>

                        {showEmail && contact.email && (
                          <div className="flex items-center gap-1 mt-1">
                            <Mail className="w-3 h-3 text-[#97CBDC]/50" />
                            <span className="text-xs text-[#97CBDC]/70 truncate">{contact.email}</span>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                {contacts.length > 10 && filteredContacts.length === 10 && (
                  <div className="px-4 py-2 text-center text-xs text-[#97CBDC]/50 border-t border-[#475B74]/30">
                    Showing first 10 results. Type to search for more specific contacts.
                  </div>
                )}
              </div>
            )}
